				...
				$.ajax({
					...
					success : function(data){
						if (data.isExist == "true") {
							...
							$("#res_Name").val(resName);
							$("#res_Code").val(resCode);
							$("#rulesUpdateDiv").html("");
							addGroup("rulesUpdateDiv");
						}
					}
				});
			} else {
...
function onClickIcon() {
	...
	/**多选标识 singleSelect 是：true ；否：false*/
	...
	window.parent.$('#commonUptFrame').attr('src', url);
	window.parent.$('#commonUptDialog').dialog({title : title});
	window.parent.$('#commonUptDialog').dialog("open");  
}
...
function setData(arr){
	...
	window.parent.$('#commonUptDialog').dialog("close");
}
...
function doSaveUptRule() {
	...
	var ruleName = $("#up_ruleName").val();
	if (ruleName == '') {
		$.messager.alert('提示', '请填入规则名称！','info');
		return;
	}
	$("#rule_Name").val(ruleName);
	var ruleDef = createJsonStr("rulesUpdateDiv");
	if (ruleDef == '') {
		$.messager.alert('提示', '请定义规则！','info');
		return;
	}
	$("#rule_Def").val(ruleDef);
	...
	$.ajax({
		url : "dataRuleController/doUpdateDataRule",
		type : "post",
		async : false,
		data : $('#ruleUptForm').serialize(),
		success : function(data) {
			window.parent.$('#ruleUptDialog').dialog("close");
			window.parent.reloadRule();
			
			window.top.msgTip("修改成功!");
		}
	});
}
/**
 ...
 * @param enName 下拉框隐藏值
 * */
function setSelect(obj,enName){
	$(obj).combobox("setValue",enName);
}
...
function openDataTalbeDlg() {
	...
	var url = "dataRuleController/toDataTableColIframe?columnsStr="+columns+"&tableName="+tableName+"&iframeId=ruleUptFrame"+"&columnsRefUser="+columnsRefUser;
