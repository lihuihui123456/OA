...
function createfloatDiv($target,data){
	...
	var top = ($target.offset().top - + getNumber($target.css("border-top-width")||1) - 1) + 'px';
	/*var left= ($target.offset().left + $target.width()
			+ getNumber($target.css("margin-left")) + getNumber($target.css("margin-right"))
			+ getNumber($target.css("padding-left")) + getNumber($target.css("padding-right"))
			+ getNumber($target.css("border-left-width")||1) + getNumber($target.css("border-right-width")||1)) + 'px';*/
	...
	$('.float-content').show();
	var html = '<input type="hidden" value="'+$.trim($target.find("a").html())+'"/>', item = '';
	var height= 38 + 22;
...
//			'	<span class="menu-item2-text">' + data[i].text + '</span><br>' +
			...
			for(var j=0;j<nodes2.length;j++){
				...
				if(node2.nodes){
					...
					c1++;
					item += ('		<span class="menu-item3" onclick="createTabFromNode(\''+node2.text+'\',\''+node2.id+'\',\''+node2.href+'\',\''+($.trim(node2.icon).length==0?icon_default:node2.icon)+'\')"><i class="icon node-icon '+($.trim(node2.icon).length==0?icon_default:node2.icon)+'"></i><br>' + node2.text + '</span>');
				}
			}
			item += '	</span>' +
			'</div>';
		...
		if($.trim($target.find("a").html())==$.trim(data[i].text)){
			...
			for(var j=0;j<nodes2.length;j++){
				...
				if(node2.nodes){
					...
					'	<span class="menu-item2-text">' + node2.text + '</span><br>' +
					'	<span class="menu-item2-content">';
					...
					for(var k=0;k<nodes3.length;k++){
						var node3 = nodes3[k];
						c2++;
						item += '		<span class="menu-item3" onclick="createTabFromNode(\''+node3.text+'\',\''+node3.id+'\',\''+node3.href+'\',\''+($.trim(node3.icon).length==0?icon_default:node3.icon)+'\')"><i class="icon node-icon '+($.trim(node3.icon).length==0?icon_default:node3.icon)+'"></i><br>' + node3.text + '</span>';
					}
					temph += 90*(c2+3)/4;
					c3++;
				}
	...
	$('.float-content').html(html).css('height',height+'px')
	.mouseleave(function(){
		$(this).hide();
		refresh();
	})
	.find('.menu-item3').each(function(){
		$(this).mouseenter(function(){
			$(this).addClass('menu-item3-color')
				.find('i').css('color','#d33238');
		}).mouseleave(function(){
			$(this).removeClass('menu-item3-color')
				.find('i').css('color','');
		});
	});
}
