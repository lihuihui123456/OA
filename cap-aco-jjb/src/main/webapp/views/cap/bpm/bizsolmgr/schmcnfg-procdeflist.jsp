...
<body class="easyui-layout" style="width:100%;height:98%">
	<!-- 页面布局西部 -->
	<div data-options="region:'west',split:true" title="流程类别" style="width:200px;">
		<!-- 流程定义类别树 -->
		<ul class="easyui-tree" id="procdsgnCtlgTree"></ul>
	</div>
	
	<!-- 页面布局中部 -->
	<div data-options="region:'center',title:'流程定义信息'">
		<!-- 列表工具栏 -->
...
function InitTableData() {
	$('#procdsgnList').datagrid({
		url : '${ctx}/procDefMgr/findMainProcdef',
		queryParams: {
			'procdsgnCtlgId':procdsgnCtlgId,
			'name': '',
			'key' : ''
		},
		method : 'post',
		...
		frozenColumns:[[    
				...
				{field  : 'id',width:40, title : '操作',
					formatter: function(value,row,index){
						return '<table border="0" width="100%"><tr>'
								+'<td><img style="cursor:pointer" src="${ctx}/views/cap/bpm/procdefmgr/img/detail.png"  title="明细" onclick="imgClick(\''+index+'\',\'detial\')"/></td>'
							+'</tr></table>';
					}
				}
            ]],
		columns : [[ 
		    ...
		    { field : 'key_', title : '标识Key', width : 80, align : 'left' }, 
		    { field : 'editorSourceValueId_', title : '状态', width : 50, align : 'left',
		    	formatter: function(value,row,index){
		    		return "已部署"
		    	}
		    }, 
...
function detail(){
	...
	if (data) {
		...
		var src = '${ctx}/procDefMgr/procdefInfoDetial?divId=dialog_&modelExtId='+modelExtId+'&deploymentId='+deploymentId;
		window.parent.$('#iframe_').attr('src', src);
		...
		$.ajax({
			...
			success : function(data){
				if(data == 'Y'){
					...
					//window.parent.document.getElementById("procdef").contentWindow.location.reload(true);
				}else{
