...
(function() {
    function v(a) {
        ...
        n = 0,
        l = 0,
        p, e = a.$.rows.length; l < e; l++) {
            p = a.$.rows[l];
            for (var d = f = 0,
            b, c = p.cells.length; d < c; d++) b = p.cells[d],
            f += b.colSpan;
            f > n && (n = f)
        }
        return n
        ...
        return {
            ...
            onLoad: function() {
                ...
                function() {
                    ...
                    c && c.setValue(a, !0);
                    a = this.getStyle("height", ""); (c = e.getContentElement("info", "txtHeight")) && c.setValue(a, !0)
                })
            },
            onShow: function() {
            ...
            onOk: function() {
                ...
                if (c.info) {
                    ...
                    if (!this.hasColumnHeaders && ("col" == f || "both" == f)) for (h = 0; h < b.$.rows.length; h++) g = new CKEDITOR.dom.element(b.$.rows[h].cells[0]),
                    g.renameNode("th"),
                    g.setAttribute("scope", "row");
                    if (this.hasColumnHeaders && "col" != f && "both" != f) for (k = 0; k < b.$.rows.length; k++) h = new CKEDITOR.dom.element(b.$.rows[k]),
                    "tbody" == h.getParent().getName() && (g = new CKEDITOR.dom.element(h.$.cells[0]), g.renameNode("td"), g.removeAttribute("scope"));
                ...
                if (this._.selectedElement) try {
                    e.selectBookmarks(d)
                } catch(p) {} else a.insertElement(b),
                setTimeout(function() {
                    ...
                    children: [{
                        ...
                        commit: function(e, d) {
                            if (this.isEnabled()) {
                                ...
                                if (b) 0 < c.count() ? (c = c.getItem(0), c.setHtml("")) : (c = new CKEDITOR.dom.element("caption", a.document), d.append(c, !0)),
    ...
    CKEDITOR.dialog.add("table",
    function(a) {
        return q(a, "table")
    });
    CKEDITOR.dialog.add("tableProperties",
    function(a) {
