...
// 数据表格行添加事件
function append(ruleid,colid, tableid,colcode) {
	var procdefid = tableid;
	var sol_id_ = colid;
	var sol_code_ = colcode;
	if (endEditing($('#'+ruleid))) {
		$('#'+ruleid).datagrid('appendRow', {
			table_id : procdefid,
			column_id : sol_id_,
			column_code:sol_code_
		});
	...
	if (row) {
		...
		if (r) {
			...
			if (row.id != '' && row.id != undefined) {
				$.ajax({
					url : "formDataRuleController/delDataRule",
					type : 'POST',
					dataType : 'json',
					data:{id:row.id},
...
function saveassessofc(ruleid,colid,tableid) {
	if (endEditing($('#'+ruleid))) {
		...
		effectRow["key"]=key;
		$.ajax({
			url : 'formDataRuleController/saveRule',
			type : 'POST',
			dataType : 'json',
			data : effectRow,
			success : function(result) {
...
function initDataDict(){	
	$.ajax({
	url:"docNumMgrController/getAllDocType",
	type:"get",
	async:false,
	dataType:"json",
	success:function(data){
		if(data!=null){
			rtemp=data["docTypeList"];
		}
	}
	});
	return rtemp;
