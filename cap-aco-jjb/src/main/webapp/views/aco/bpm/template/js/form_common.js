...
function btnSave(bizId, action, operate) {
	//保存业务基本信息
	var data = checkBizInfo(action, operate);
	if(data != "Y") {
		if(action == "save") {
			layerAlert("业务基本信息保存出错，暂存失败！");
		}else if(action == "send"){
			layerAlert("业务基本信息保存出错，送交失败！");
		}
		return false;
	}
	//保存业务表单信息
	var result = $("#form_iframe")[0].contentWindow.doSaveForm(bizId, operate);
	if(result != "Y"){
		if(action == "save") {
			layerAlert("业务信息保存出错，暂存失败！");
		}else if(action == "send") {
			layerAlert("业务信息保存出错，送交失败！");
		}
		return false;
	}
	return true;
 ...
 */
var iseb = "N";  
function checkBizInfo(action, operate) {
	var flag = "Y";
	if(action == "save" || action == "send"){
		flag = doSaveBpmRuBizInfo(operate);
		if(flag == "Y"){
			iseb = "Y";
		}
	}else {
