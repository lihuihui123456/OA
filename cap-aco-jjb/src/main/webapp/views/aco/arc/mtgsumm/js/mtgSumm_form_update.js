...
function doReset(){
  document.getElementById("mtgSummFormUpdate").reset();
}
function doSaveFormUpdate(obj) {
	var flag = "N"
	if ($('#mtgSummFormUpdate').validationEngine('validate')) {
		var isAtt = obj.contentWindow.isNull();
		if(isAtt==false){
			layerAlert("请上传会议纪要！");
			return;
		}
		$.ajax({
			type : "POST",
			url : "arcMtgSumm/update",
			data : $('#mtgSummFormUpdate').serialize(),
			success : function(data) {
			},
			error : function(data) {
...
function validateForm(){
	return $('#mtgSummFormUpdate').validationEngine('validate');
}
