...
<script src="${ctx}/views/aco/arc/borrinfor/js/borrInforModify.js"></script>
<script
...
<script type="text/javascript">
	var globalCTX = '${ctx}';
	var showBorrisSet = '${showBorrisSet}';
	var showBorrisBack = '${showBorrisBack}';
	 $(function(){
		 $("#arcName").focus();
	});
	...
	function saveAttach(){
		...
		//return myArray;
		$('<input name="serialNumber" id="fileattid" type="hidden" value="'+myArray[2]+'">').appendTo('#borrInforAttList');
		$('<input name="id" id="'+myArray[2]+'_i" type="checkbox" disabled="disabled" checked="checked" value="'+myArray[2]+'">').appendTo('#borrInforAttList');
		$('<a id="'+myArray[2]+'_a" href="/cap-base-arc/media/download?documentId='+myArray[2]+'">'+myArray[3]+'</a>').appendTo('#borrInforAttList');
		//var temp = [{'attId':myArray[2],'arcName':myArray[1],'attName':myArray[3]}];
		$('#borrInforForm').find('#arcId').val(myArray[0]);
		$('#borrInforForm').find('#attId').val(myArray[2]);
		return myArray;
	}
	...
	<div class="paper-inner">
		<!-- input tab -->
			<form id="borrInforForm" enctype="multipart/form-data" class="form-horizontal " target="_top">
				...
				<input id="attId" type="hidden" name="attId" value="${show.borrInfor.attId }">
				<input id="id" type="hidden" name="id" value="${show.borrInfor.id }">
				<!-- each input -->
						...
						<td>
							<div id="borNBRDiv" class="" style="margin-left: 0; margin-right: 0">
									<input type="text" id="borNBR" name="borNBR" readonly="readonly"
										class="form-control  input" value="${show.borrInfor.borNBR }">
							</div>
						</td>
						<th>单号日期</th>
						<td>
							<div id="nbrTimeDiv" class="" style="margin-left: 0; margin-right: 0">
								<input type="text" id="nbrTime" name="nbrTime" value="${show.borrInfor.nbrTime}" class="form-control " onclick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm:ss'})">
							</div>
						</td>
					</tr>
					<tr>
...
<%-- 								<select class="selectpicker select validate[required]" style="width: 100%;" id="borrDept"
									name="borrDept" value="" myvalue="${show.borrInfor.borrDept }" title="请选择">
									<c:forEach items="${ borrDept}" var="item">
										...
										<input id="jieyueBumenId_" class="form-control" name="borrDept" type="hidden" style="width: 100%; height: 29; border: 0;" value="${show.borrInfor.borrDept }" />
										<input id="jieyueBumenIdName_" name="borrDeptName" class="validate[required] form-control select" type="text" 
											...
											onclick="peopleTree(2,'jieyueBumenId_');" value="${show.jieyueBumen }" /> 
							</div>							
						</td>					
...
<%-- 									<input type="text" id="borrUser" name="borrUser" value="${show.borrInfor.borrUser }" class="form-control input validate[required]"  > --%>
									...
									type="hidden" style="width: 100%; height: 29; border: 0;" value="${show.borrInfor.borrUser }" />
									<input id="checkUserIdName_" name="borrUserName"
									class="validate[required] form-control select"  
									onclick="peopleTree(1,'checkUserId_')"; 
									 type="text" style="width: 100%; height: 29; border: 0;"
									value="${show.borrUserName }" />							
							</div>
						</td>
