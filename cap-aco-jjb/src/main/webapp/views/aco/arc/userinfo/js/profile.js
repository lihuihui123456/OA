function profile(){
	var options={
		"text":"用户中心",
		"id":"grxx"+1240,
		"href":"arcUserController/findPerInfoById",
		"pid":window
	};
...
function changeSave(){
	$.ajax({
		url:'arcUserController/changeData',
		data:$('#ff').serialize(),
		type:'post',
...
$(function(){
	$('#appUpload').click(function(){
		var options = {
				"text" : "立即下载",
				"href" : "views/arc/userinfo/index.jsp",
				"pid":window
		};
		window.parent.createTab(options);
	});
...
function validPWd(){
	...
	$.ajax( {
		url : 'arcUserController/validPWd',
		data:{pwd:pass},
		async: false,//同步
		cache: false,
...
function submitForm(){
	...
	$.ajax({
		url:'arcUserController/updatePwd',
