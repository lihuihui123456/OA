...
<script src="${ctx}/views/aco/arc/prj/list/js/arcprj-list.js"></script>
...
<script type="text/javascript">
	 ...
	 */
	 function expExcel(){
			//判断是否有选中
			var obj = $('#arcPrjList').bootstrapTable('getSelections');
			if (obj.length!=''||obj.length==1) {
				...
				$("#selectIds").val("");
					...
					if($("#input-word").val()!="请输入项目名称查询"&&$("#input-word").val()!=""){
					 ...
					 window.location.href = "${ctx}/arcPrjController/exportPrjExcel?"+$("#search_form").serialize();
					if($("#input-word").val()!="请输入项目名称查询"&&$("#input-word").val()!=""){
						 $("#sproName").val("");
					}	
	 }
