...
function doSaveForm(arcId) {
	$('#arcId').val(arcId);
	var flag = "N"
	if ($('#arcDclrForm').validationEngine('validate')) {
		var decMny = $("#decMny").val();
		if (!/^[0-9]*$/.test(decMny)) {
			layerAlert("请输入数字!");
			flag = "Y";
		} else {
			$.ajax({
				type : "POST",
				url : "arcDclrController/doAddArcDclrInfo",
				data : $('#arcDclrForm').serialize(),
				success : function(data) {
					layerAlert("保存成功!");
				},
				error : function(data) {
...
function doUpdateSaveForm(arcId) {
	...
	if ($('#arcDclrForm').validationEngine('validate')) {
		...
		if (!/^[0-9]*$/.test(decMny)) {
			...
			$.ajax({
				type : "POST",
				url : "arcDclrController/doUpdateSaveArcDclrInfo",
				data : $('#arcDclrForm').serialize(),
				success : function(data) {
					layerAlert("保存成功!");
				},
...
function clearCUserForm(){
	  document.getElementById("arcDclrForm").reset();
