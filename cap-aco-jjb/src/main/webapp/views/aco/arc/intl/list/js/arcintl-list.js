$(function() {
	...
	$('#add_btn').click(function() {
		...
		var options = {
			"text" : "内部项目-登记",
			"id" : date,
			"href" : "arcIntlController/goToArcIntlAdd?typeId="+typeId,
			"pid" : window.parent
		};
	...
	$('#update_btn').click(function() {
		var typeId = $('#typeId').val();
		var selects = $("#arcIntlList").bootstrapTable('getSelections');  
		var arcId = "";
		var fileStart = "";
		if(selects.length==0){
			...
			var options = {
				"text" : "内部项目-修改",
				"id" : date,
				"href" : "arcIntlController/goToArcIntlUpdate?arcId="+arcId+"&typeId="+typeId+"&type=update&fileStart="+fileStart,
				"pid" : window.parent
			};
			window.parent.parent.createTab(options);
		}
			...
			$.ajax({
				...
				success : function(data) {
					if(data=="true"){
						...
						/*$("#arcIntlList").bootstrapTable('refresh');*/
						refreshTable('arcIntlList','arcIntlController/findAllArcIntlDataPage');
					}
				},
				error : function(data) {
					history.go(0);
				...
				success : function(data) {
					if(data=="true"){
						...
						var options = {
							"text" : "内部项目-追加归档",
							"id" : date,
							"href" : "arcIntlController/goToArcIntlUpdate?arcId="+arcId+"&typeId="+typeId+"&type=addFile",
							"pid" : window.parent
						};
						window.parent.parent.createTab(options);
					}
...
/*			layer.confirm('确定删除吗？', {
	...
	$('#view_btn').click(function(){
		...
		var options = {
				"text" : "内部项目-查看",
				"id" :arcId,
				"href" : "arcIntlController/goToArcIntlView?arcId="+arcId+"&fileStart="+fileStart+"&type=view",
				"pid" : window.parent
			};
			window.parent.parent.createTab(options);
	});
 ...
 */
function initTable() {
	$('#arcIntlList').bootstrapTable({
		url : 'arcIntlController/findAllArcIntlDataPage', // 请求后台的URL（*）
		method : 'get', // 请求方式（*）
...
*/ 		    formatter: onTdClickTabFormatter
		...
		onClickRow: function (row, tr) {
			var options = {
					"text" : "内部项目-查看",
					"id" : row.arc_id,
					"href" : "arcIntlController/goToArcIntlView?arcId="+row.arc_id+"&fileStart="+row.file_start+"&type=view",
					"pid" : window.parent
				};
				window.parent.parent.createTab(options);
		}/*,
...
function searchCommon() {
	...
	$("#arcIntlList").bootstrapTable('refresh',{
		url : "arcIntlController/findAllArcIntlDataPage",
		query:{
