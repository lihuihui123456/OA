		...
		<form id="myFm">
			...
			<table class="tablestyle" width="100%" border="0" cellspacing="0">
				<tr>
					...
					<td style="width: 30%;">
						<input class="validate[required] form-control select" readonly="readonly" id="userName" name="createUserName" type="text" style="width: 100%; height: 29; border: 0;" value='${userName}' />
					</td>
					<th>创建时间</th>
					<td style="width: 30%;">
						<input id="createTime"  name="createTime" type="text" readonly="readonly" class="form-control" value='${createTime}' />
					</td>
				</tr>
				<tr>
					<th>部门</th>
				...
				<tr>
					...
					<td colspan="3">
						<select  id="proName" name="proName" class="form-control" size="1" >
							<option value=""></option>
						    <option <c:if test="${proName==0}"> selected="selected"</c:if>  value="0">"双优"评选项目</option>
							<option <c:if test="${proName==1}"> selected="selected"</c:if>  value="1">"丹柯杯"项目</option>
                            <option <c:if test="${proName==2}"> selected="selected"</c:if>  value="2">基本经费</option>
						</select>
					</td>
				</tr>
				<tr>
					...
					<td colspan="3">
						<select  id="funcType" name="funcType" class="form-control" size="1" >
							<option <c:if test="${funcType==0}"> selected="selected"</c:if>   value="0">工资福利支出</option>
		    				<option <c:if test="${funcType==1}"> selected="selected"</c:if>   value="1">商品和服务支出</option>
		    				<option <c:if test="${funcType==2}"> selected="selected"</c:if>   value="2">对个人和家庭的补助</option>
		    				<option <c:if test="${funcType==3}"> selected="selected"</c:if>   value="3">对企事业单位补助</option>
		    				<option <c:if test="${funcType==4}"> selected="selected"</c:if>   value="4">转移性支出</option>
	    					<option <c:if test="${funcType==5}"> selected="selected"</c:if>   value="5">债务利息支出</option>
		    				<option <c:if test="${funcType==6}"> selected="selected"</c:if>   value="6">债务还本支出</option>
		    				<option <c:if test="${funcType==7}"> selected="selected"</c:if>   value="7">其他资本性支出</option>
		    				<option <c:if test="${funcType==8}"> selected="selected"</c:if>   value="8">其他支出</option>
						</select>
					</td>
				</tr>
				<tr>
					<th>预算总金额</th>
					<td colspan="3">
						<input id="budgetAmount" name="budgetAmount" type="text" class="form-control " value='${je}' />
					</td>
				</tr>
				<tr>
					<th>备注</th>
					<td colspan="3">
						<input type="text" id="remarks" name="remarks" class="form-control " value='${remarks}' />
					</td>
				</tr>
			</table>
		</form>
