function WebOffice2015() {
    ...
    */
    ...
    this.WebOpen = function (mBoolean) {
        ...
        this.WebSetMsgByName("FilePath", this.FilePath);
    ...
    this.WebSave = function () {
        ...
        this.WebSetMsgByName("FILENAME", this.FileName); //加载FileName
        this.WebSetMsgByName("FilePath", filepath);
     ...
     * 隐藏痕迹时之前的痕迹不受影响
     */
     this.WebShow = function (blnValue,blnValue2) {
         if (this.getDocType(this.FileType) == this.DocType.WOED) {
             this.VBAShowRevisions(blnValue,blnValue2);
         }
     }
  ...
  //光标处添加书签
   ...
   //删除书签
    ...
    this.DownloadToFile = function (DownFileName, SavePath) {
        ...
        httpclient.ShowProgressUI = false;//隐藏进度条
        //模板路径
        if (httpclient.Open(this.HttpMethod.Get, URL + "/views/cap/sys/iweboffice/template/" + DownFileName, false)) {//指定下载模板的名称
            if (httpclient.Send()) {
                if (httpclient.Status == 200) {
    ...
    */
    //显示痕迹和隐藏痕迹
    this.VBAShowRevisions = function (mValue,mValue2) {
        ...
        this.obj.ActiveDocument.ShowRevisions = mValue2; //显示痕迹或隐藏
	    	return true;
    }
    	return false;
     }
    ...
    */
    ...
    this.AddToolbar=function() {
    	this.obj.Style.ShowCustomToolbar=false;
    	this.obj.Handwritting.Show = !this.obj.Handwritting.Show;    	
    	var URL = this.WebUrl.substring(0, this.WebUrl.lastIndexOf("/"));
	    var customtoolbar = this.obj.CustomToolbar;
	    if(this.EditType=="0"){
	    	customtoolbar.AddToolButton(309,"打印文档","", "打印文档",0);
	    	customtoolbar.AddToolButton(4,"打印预览","", "打印预览",0);
	    }else{
	    	customtoolbar.AddToolButton(307,"保存文档","", "保存文档",0);
	    	customtoolbar.AddToolButton(308,"另存文档","", "另存文档",0);
	    	customtoolbar.AddToolButton(309,"打印文档","", "打印文档",0);
	    	customtoolbar.AddToolButton(311,"显示痕迹","", "显示痕迹",0);
	    	customtoolbar.AddToolButton(312,"隐藏痕迹","", "隐藏痕迹",0);
	    	customtoolbar.AddToolButton(314,"套红","", "套红",0);
	    	customtoolbar.AddToolButton(301,"手写签名","", "手写签名",0);
	    	customtoolbar.AddToolButton(302,"停止手写","","停止手写",0);
	    	customtoolbar.AddToolButton(303,"文字签名","","文字签名",0);
	    	customtoolbar.AddToolButton(304,"图形签名","","图形签名",0);
	    	customtoolbar.AddToolButton(305,"取消上一个签批","","取消上一个签批",0);
	    	customtoolbar.AddToolButton(306,"显示用户签批","","显示用户签批",0);
	    	customtoolbar.AddToolButton(609,"签章","", "签章",0);
	    }
