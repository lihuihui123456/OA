$(function() {
	...
	//set select value
	var selects = $('#arcBidForm').find('select');
	for(var i=0;i<selects.length;i++){
		...
		if(option!=null&&option.length==1){
			...
			//$(selects[i]).selectpicker('val',selectValue);
		}
	}
	$('#guidangDiv').find('input').attr('readonly',true).attr('disabled',true);
	var type = $("#type").val();
	if(type=="addFile"){
		$("input,select,textarea",$("form[name='arcBidForm']")).attr("disabled","disabled");
	}
...
function doSaveForm(arcId) {
	$('#arcId').val(arcId);
	var flag = "N"
	if ($('#arcBidForm').validationEngine('validate')) {
		$.ajax({
			type : "POST",
			url : "arcBidController/doAddArcBidInfo",
			data : $('#arcBidForm').serialize(),
			success : function(data) {
				return flag;
			},
			error : function(data) {
...
function doUpdateSaveForm(arcId) {
	...
	if ($('#arcBidForm').validationEngine('validate')) {
		$.ajax({
			type : "POST",
			url : "arcBidController/doUpdateSaveArcBidInfo",
			data : $('#arcBidForm').serialize(),
			success : function(data) {
				return flag;
			},
...
function validateForm(){
	return $('#arcBidForm').validationEngine('validate');
}
function clearCUserForm(){
	  document.getElementById("arcBidForm").reset();
