...
function doSaveForm(arcId) {
	$('#arcId').val(arcId);
	var flag = "N"
	if ($('#arcInvForm').validationEngine('validate')) {
		$.ajax({
			type : "POST",
			url : "arcInvController/doAddArcInvInfo",
			data : $('#arcInvForm').serialize(),
			success : function(data) {
			},
			error : function(data) {
				layerAlert("error:" + data.responseText);
...
function doUpdateSaveForm(arcId) {
	...
	if ($('#arcInvForm').validationEngine('validate')) {
		$.ajax({
			type : "POST",
			url : "arcInvController/doUpdateSaveArcInvInfo",
			data : $('#arcInvForm').serialize(),
			success : function(data) {
			},
			error : function(data) {
...
function validateForm() {
	return $('#arcInvForm').validationEngine('validate');
}

function clearCUserForm() {
	  document.getElementById("arcInvForm").reset();
