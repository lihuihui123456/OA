...
<link
	...
	rel="stylesheet">
<link
	href="${ctx}/static/cap/plugins/bootstrap/plugins/laydate-v1.1/need/laydate.css"
	rel="stylesheet">
<link
	href="${ctx}/static/cap/plugins/bootstrap/plugins/laydate-v1.1/skins/default/laydate.css"
	rel="stylesheet">
...
<script src="${ctx}/views/aco/arc/mtgsumm/js/mtgSummIndex.js"></script>
<script type="text/javascript">
...
$(document).ready(function() {
	...
	$('#folderContent').bootstrapTable({
		...
		queryParams : function(params) {
			validateData();
		    var	amsName="";
			if($("#input-word").val()!="请输入会议名称查询"&&$("#input-word").val()!=""){
				amsName=amsName+$("#input-word").val();
				$("#search_form")[0].reset();
			}
			amsName=amsName+$("#amsName").val();
			var temp = {// 这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
				...
				yearNum:$("#searchregYear").val(),
				expiryDate:$("#expiryDate").val(),
				startRegTime:$("#startTime").val(),
				endRegTime:$("#endTime").val(),
 ...
 */			};
		...
		uniqueId : "arcId", // 每一行的唯一标识，一般为主键列
		showToggle : false, // 是否显示详细视图和列表视图的切换按钮
		cardView : false, // 是否显示详细视图
		detailView : false, // 是否显示父子表
		singleSelect : true,
		columns : [ [ 
 			 { checkbox : true ,field:'checkStatus'}, 
 ...
 */ 		        formatter: onTdClickTabFormatter}, 
 		    { field : 'ams_name', title : '会议名称',sortable : true},
 		    { field : 'ams_time', title : '会议时间',
				align : 'center',
				valign: 'middle',
				sortable : true ,
 			formatter:function(value,row){
 				if(row.ams_time==null){
	 				  return "--";
	 				}
	 				else{
	 					return new Date(parseInt(row.ams_time)).Format('yyyy-MM-dd ');	
	 				}
							}
 		    }, 
		    { field : 'ams_emcee', title : '主持人',sortable : true},
		...
		onClickRow: function (row, tr) {
 		...
 		responseHandler:function(res){
 			$.each(res.rows, function (i, row) {
 				 row.checkStatus  = $.inArray(row.arcId, selectionIds) !== -1;
 				});
 				return res;	
 		}
	});
...
//字段排序
var attribute='ams_name';
var orderBy='DESC';
function upMake(attributeParam){
	attribute=attributeParam;
	orderBy='ASC';
}
function downMake(attributeParam){
	attribute=attributeParam;
	orderBy='DESC';
}
function search() {
...
/**
	...
	if (obj.length!=''||obj.length==1) {
		...
		$("#selectIds").val("");
		 ...
		 window.location.href = "${ctx}/arcMtgSumm/exportExcel?"+$("#search_form").serialize();
			if($("#input-word").val()!="请输入会议名称查询"&&$("#input-word").val()!=""){
				 $("#amsName").val("");
			}
	 }
...
$(function(){
	 ...
	 */
	 var union = function(array,ids){  
	     $.each(ids, function (i, arcId) {  
	         if($.inArray(arcId,array)==-1){  
	             array[array.length] = arcId;  
	         }  
	     });  
	      return array;  
	};
...
<body>
	...
	<div style="padding-top: 0px;padding-bottom:0px;border:0;">
					<div id="search_div" style="padding: 10px 0px;width: 300px; float: right">
					<div class="input-group">
						<input type="text" id="input-word" class="form-control input-sm"
							value="请输入会议名称查询" onFocus="if (value =='请输入会议名称查询'){value=''}"
							onBlur="if (value ==''){value='请输入会议名称查询'}"> 
		...
		<div id="toolbar" class="btn-group" style="margin-top:10px;">
			...
			<button id="filestart_btn" type="button"
				class="btn btn-default btn-sm">
				<span class="fa fa-files-o" aria-hidden="true"></span> 归档
			</button>
			<button id="addfilestart_btn" type="button"
				class="btn btn-default btn-sm">
...
<%-- 		<div id=""
			style="border:1px solid #ddd; background:#fff; padding-top:10px; padding-right:0%; margin-top: 10px">
			<form class="form-horizontal" id="search_form"">
				<input type="hidden" id="amsType" name="amsType" value="${typeId}" />
				<div class="form-group">
					<label class="control-label col-lg-2 col-md-2 col-sm-2 col-xs-2">会议名称:</label>
					<div class="col-lg-4  col-md-4 col-sm-4 col-xs-4">
						<input class="form-control input-sm" type="text" id="amsName"
							name="amsName" placeholder="请输入会议名称" style="margin-left: -50px;">
					</div>
					<label
						class="control-label col-lg-2 col-lg-2 col-md-2 col-sm-2 col-xs-2">档案状态:</label>
					<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
						<select id="fileStart" name="fileStart"
							class="selectPiker select input-sm"
							style="color: #000;width: 100%;margin-left: -50px;">
				...
				<div class="form-group">
					...
					<div class="col-lg-4  col-md-4 col-sm-4 col-xs-4">
						<select class="form-control input-sm selectpicker"
							id="searchregYear" name="searchregYear" value="" title="请选择年度"
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<div class="modal-body">
					<iframe id="group" runat="server" width="100%" height="350"
			...
			<div class="modal-content">
			   ...
			   <form class="form-horizontal" id="search_form"">
				...
				<div class="form-group">
					...
					<!-- 					<label
								class="control-label col-lg-2 col-lg-2 col-md-2 col-sm-2 col-xs-2">会议类型:</label>
							<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
								<select id="amsType" name="amsType" class="selectPiker select input-sm" style="color: #000;width: 100%;padding: 5px 10px;">
