...
<script src="${ctx}/views/aco/arc/intl/list/js/arcintl-list.js"></script>
...
<script type="text/javascript">
	 ...
	 */
	 function expExcel(){
			//判断是否有选中
			var obj = $('#arcIntlList').bootstrapTable('getSelections');
			if (obj.length!=''||obj.length==1) {
				...
				$("#selectIds").val("");
					...
					if($("#input-word").val()!="请输入项目名称查询"&&$("#input-word").val()!=""){
					 ...
					 window.location.href = "${ctx}/arcIntlController/exportIntlExcel?"+$("#search_form").serialize();
					if($("#input-word").val()!="请输入项目名称查询"&&$("#input-word").val()!=""){
						 $("#sproName").val("");
					}	
	 }
