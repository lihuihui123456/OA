...
<script src="${ctx}/views/aco/templatemgmt/js/WebOffice.js"></script>
	...
	function SaveDocument(){
	  if (WebOffice.WebSave()){    //交互OfficeServer的OPTION="SAVEFILE"
		  if($("#iWebOfficeForm").validationEngine('validate')){
			  $.ajax({
					type : "POST",
					url : '${ctx}/templateMgmt/doSaveTemplate',
					dataType : 'json',
...
<form class="form-horizontal" id="iWebOfficeForm" method="post" action="">
	<div class="form-group">
		<label class="control-label col-lg-2 col-md-2 col-sm-2 col-xs-2"><span class="star" style="color: red;">*</span>模板名</label>
		...
		<label class="control-label col-lg-2 col-md-2 col-sm-2 col-xs-2">说明</label>
		<div class="col-lg-4  col-md-4 col-sm-4 col-xs-4">
	...
	<div class="form-group">
		<label class="control-label col-lg-2 col-md-2 col-sm-2 col-xs-2"><span class="star" style="color: red;">*</span>模板类型</label>
			<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
					<select id="difType_" name="difType_" class="selectPiker select input-sm" style="color: #333;width: 100%;padding: 0px 10px;">
						<!-- <option value=" ">全部</option> -->
						<option <c:if test="${template.difType_==' '}"> selected="selected" </c:if> value=" ">全部</option>
						<option <c:if test="${template.difType_=='0'}"> selected="selected" </c:if> value="0">套红模板</option>
						<option <c:if test="${template.difType_=='1'}"> selected="selected" </c:if> value="1">打印模板</option>
						<option <c:if test="${template.difType_=='2'}"> selected="selected" </c:if> value="2">电子签章模板</option>
				<!-- 		<option value="0">套红模板</option>
						<option value="1">打印模板</option>
						<option value="2">电子签章模板</option> -->
					</select>
			</div>
	</div>
	<div style="text-align:center;margin-left:5%;">
		...
		<button type="button"  class="btn btn-default btn-sm" title="提示 : 制作模板时,请根据需要在word中设置书签,如为正文设置书签:Content,标题:title,编号:number,主送:mainsend,抄送:copysend。在word中设置书签的具体方法参考http://jingyan.baidu.com/article/4f7d57128c10411a21192766.html">
...
function HelpDocument(){
	  alert("提示：请在模板中根据需要设置书签,如正文:Content,标题：title,编号:number,主送:mainsend,抄送:copysend");
  }
 </script>