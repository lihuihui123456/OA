...
$(function() {
	...
	$("#mt_btn_openDialog").click(function() {
		$("#dtlist").bootstrapTable('refresh',{
			url:'bizMaterialStockController/findAllStock'});
		idss.length = 0;
		//弹出模态窗口
		$('#myModal').modal({
			backdrop : 'static',
...
function submitForm(action){
	...
	$(allData).each(function(index) {
		...
		var innumber = allData[index].amount_;
	...
	$.ajax({
		url : 'bizMaterialApplyController/doUpdateMaterialApply/' + material+'/'+action,
		type : "POST",
		dataType : 'json',
		data:$("#ff").serialize(),
