	...
	<!-- 		<button id="scan" type="button" style="display: none" class="btn btn-default btn-sm" onclick="scan()">
		...
		<div id="tabsH">
			...
			<ul id="myTabs" class="nav nav-tabs" role="tablist">
				...
				<c:if test="${bean.isEarc== '1' }">
					<li role="presentation">
						<a id="glwd" href="#earc" onclick='showPagel("earc")' role="tab" data-toggle="tab">档案信息</a>
					</li>
				</c:if>
			</ul>
			<!-- 页签头结束 -->
			...
			<div class="tab-content" id="panel_box">
				...
				<c:if test="${bean.isEarc== '1' }">
					<div role="tabpanel" class="tab-pane fade in" id="earc">
						<iframe id="earc_iframe" runat="server" src="" height="450px;" width="100%"
					 frameborder="no" border="0" scrolling="yes"></iframe>
					</div>	
				</c:if>
			</div>
...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#btn_save").click(function() {
			...
			if(btnSave("${bizId}","save","${operateState}")) {
				...
				//保存电子档案业务信息保存
				//获取业务标题
				var  receiverUser = $("#form_iframe")[0].contentWindow.getJsr_();
				var  sendUser = $("#form_iframe")[0].contentWindow.getFsr_();
				var  startDate = $("#form_iframe")[0].contentWindow.getKssj_();
				var  endDate = $("#form_iframe")[0].contentWindow.getJssj_();
				alert(receiverUser+"==="+sendUser+"=="+startDate+"==="+endDate);
				btn_save_earc_seddRead("${bizId}",receiverUser,sendUser,startDate,endDate);
				//关闭当前页面并刷新父页面
				closeWindow(true);
			}
		});
	...
	function showPagel(typel) {
		if(typel=="mainBody") {//mainBody
			...
			var attachmentURL = "media/toEarcMgrAtch?bizId="+"${bizId}";
			showPage(typel,attachmentURL);
		}
	}
