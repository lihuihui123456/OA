...
function onEndEdit(index, row) {
	...
	/*
	 * if(ed!=null){ row.user_type_ = $(ed.target).combobox('getText'); }
	 */

}

// 数据表格行添加事件
function append(ruleid,colid, tableid) {
	...
	if (row) {
		...
		if (r) {
			...
			if (row.id != '' && row.id != undefined) {
				$.ajax({
					url : "formUserRuleController/delUserRule",
					type : 'POST',
					dataType : 'json',
					data:{id:row.id},
...
function saveassessofc(ruleid,colid,tableid) {
	if (endEditing($('#'+ruleid))) {
		...
		effectRow["key"]=key;
		var list=$('#'+ruleid).datagrid('getData')["rows"];
		if(list!=null&&list.length>0){
			for(var i=0;i<list.length;i++){
				var row=list[i];
				if(row["control_type"]==null||row["control_type"]==""){
					$.messager.alert('提示', '权限类型不能为空！');
					return;
				}
				if(row["rule_type"]==null||row["rule_type"]==""){
					$.messager.alert('提示', '用户类型不能为空！');
					return;
				}
				if(row["rule_id"]==null||row["rule_id"]==""){
					$.messager.alert('提示', '用户值不能为空！');
					return;
				}
			}
		}else{
		...
		$.ajax({
			url : 'formUserRuleController/saveRule',
			type : 'POST',
			dataType : 'json',
			data : effectRow,
			success : function(result) {
...
function onBeforeEdit(index, row) {
	if (row != null) {
		if (row.rule_type == '1') {
			 ...
			 * else{ row.ruleid_value = ''; row.ruleid=''; }
			 */
	}
}
...
function onClickIcon() {
	...
	if (row.rule_type != '1') {
		...
		/*
		 * if (row.ruletype == '3') { $("#choose_user").attr("src",
		 * '../userGroupController/selectUserGroup'); } else {
		 * $("#choose_user").attr("src",
		 * '../roleUserController/selectRoleUser'); }
		 */
		$('#chooseper').dialog({
			title : '用户选择',
			width : 620,
